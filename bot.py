...
def select_company(update: Update, context: CallbackContext):
    ...
    response = requests.post(VOICE_API_URL, headers=headers, json=data)
...
# Handle /otp command
def generate_otp_manually(update: Update, context: CallbackContext):
    otp = generate_otp()
...
def main():
    ...
    dp.add_handler(CommandHandler("otp", generate_otp_manually))
